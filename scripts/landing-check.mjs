...
for (const v of VIEWS) {
  ...
  await page.goto("http://127.0.0.1:3210/landing", { waitUntil: "networkidle" });
  await page.waitForTimeout(1300);
  await page.screenshot({ path: `.visual/lp-${name}-hero.png` });

  const m = await page.evaluate(() => {
    const vh = window.innerHeight;
    const btn = document.querySelector("[data-cta-anchor] .btn");
    ...
    return {
      ...
      smallTargets,
      stickyShown: document.querySelector(".dock")?.dataset.shown,
    };
  });
  ...
  if (name !== "desktop") {
    ...
    const shown = await page.evaluate(() => document.querySelector(".dock")?.dataset.shown);
    ...
    const hidden = await page.evaluate(() => document.querySelector(".dock")?.dataset.shown);
  ...
  const stuck = await page.evaluate(() =>
    [...document.querySelectorAll(".rise:not(.is-in)")].map((el) => {
...
for (const route of ["/", "/pricing", "/settings", "/test"]) {
  await page.goto(`http://127.0.0.1:3210${route}`, { waitUntil: "networkidle" });
